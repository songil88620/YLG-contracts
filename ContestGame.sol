...
contract ContestGame {
    ...
    //
    mapping(address => mapping(string => bool)) public matchActive; 


    event MatchCreated(address player1, address player2, string category, uint matchID, uint tournamentID, bool finished, uint creationTime);
    ...
    event TournamentCreated(string category, uint tournamentID, uint maxPlayers, uint tournamentFee, string uri, uint period, uint startTime, uint creationTime);
    event TournamentFeePaid(address player, uint tournamentID, uint256 tournamentFee, uint256 count, uint settedTime);
    event MinTokensStakedPlayUpdated(uint minYLTStaked, uint settedTime);
    event MatchActived(address gamer, address subvault, string category, bool state);
    ...
    // Function to active match for gamer
    function activeMatch(string memory _category, address subvault) public {
        matchActive[msg.sender][_category] = !matchActive[msg.sender][_category]; 
        emit MatchActived(msg.sender, subvault, _category, matchActive[msg.sender][_category]);
    } 
    ...
    function createMatchID(address _player1, address _player2, string memory _category, uint tournamentID) public {
        require(ylProxy.getRole(msg.sender) == 8, "You don't have permission");
        require(matchActive[_player1][_category], "player1 didn't active match");
        require(matchActive[_player2][_category], "player1 didn't active match");
        if(tournamentID > 0) {
            require(ylProxy.getRole(msg.sender) == 8, "You don't have permission"); 
        } 
        ...
        emit MatchCreated(_player1, _player2, _category, matchCount, tournamentID, false, block.timestamp);        
        matchCount++;
       // matchActive[_player1][_category] = false;
       // matchActive[_player2][_category] = false;
    }

    // Play function, If we leave the _tournament parameter as 0 it will take it as a daily game. 
    function play(address _player1, uint _score1, address _player2, uint _score2, uint _matchID, string memory _category, uint8 _tournamentID) public { 
        require(ylProxy.getRole(msg.sender) == 8, "You don't have permission");
    ...
    function createTournament(string memory _category, uint _maxPlayers, uint256 _tournamentFee, string memory _uri, uint period, uint startTime ) external {
        require(ylProxy.getRole(msg.sender) == 8, "You don't have permission");
        ...
        emit TournamentCreated(_category, tournamentCount, _maxPlayers, _tournamentFee, _uri, period, startTime, block.timestamp);      
        tournamentCount++;  
    }
    ...
    function payTournamentFee(uint8 _tournamentID) external {
        ...
        emit TournamentFeePaid(msg.sender, _tournamentID, tournamentIndex[_tournamentID].totalPaidFees, tournamentIndex[_tournamentID].players.length, block.timestamp);
    }
