...
interface IProxy {
    ...
    function getComissionByUser(address user) external view returns(uint256[] memory);
    function getGroupAssign(address user) external view returns(address);
    function updateEscrowAmount(address user, uint256 amoun) external;
    function updateNftSalePrice(uint256 nftid, uint256 salepirce) external;
    function getNftSalePrice(uint256 nftid) external view returns(uint256);
}
    ...
    ReentrancyGuard
{
    ...
    event minted721(address indexed minter, uint256 tokenId,  string tokenUri, uint256 mintedGas);
    ...
    event TokenUpdated(address editor, uint256 tokenId, string tokenUri, uint256 timestamp);
    ...
    //mint
    function createToken( string memory tokenURI, string memory _sport, string memory _cnft, address _actor ) public returns (uint256) {
        uint256 startGas = gasleft(); 
        require(
            proxy.isMintableAccount(msg.sender) || proxy.getRole(msg.sender) == 5,
            "you can't mint YLT NFT, please contact the Admin"
        );
        //if( proxy.getRole(msg.sender) == 5 && bytes(_sport).length != 0){
        ...
        emit minted721(msg.sender, newTokenId, tokenURI, gasUsed);
    ...
    //lazy minting part
    function lazyCreateToken( string memory _tokenURI, string memory _sport, string memory _cnft, address _actor) public onlyOwner payable returns (uint256) {
        require( categoryAmount[_sport][_cnft] >= (categoryCount[_sport][_cnft]) + 1, " Overflow! Amount of NFT category" );
        require(getPauseContract() == false, "NFT721 Contract was paused!");  
        ...
        emit minted721(msg.sender, newTokenId, _tokenURI, 0);
    ...
    function updateTokenURI(address editor, uint256 tokenId, string memory tokenURI) public { 
        require(ownerOf(tokenId) == msg.sender, "you are not owner of this token");
        require(_exists(tokenId), "Token does not exist");
        _setTokenURI(tokenId, tokenURI);
        emit TokenUpdated(editor, tokenId, tokenURI, block.timestamp);
    }
